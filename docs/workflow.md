...
Работа начинается с диалога в Claude Code: `/aif-explore` исследует кодовую базу и обсуждает что нужно сделать. Из обсуждения появляются задачи-кандидаты.

`/roadmap-decompose` берёт эти задачи и декомпозирует их до атомарного уровня — каждый milestone должен делать одну вещь и описывать её достаточно точно, чтобы Planner не угадывал намерения. Цикл `/aif-explore` → `/roadmap-decompose` повторяется несколько раз, пока задачи не становятся атомарными и самодостаточными.
...
```
/aif-explore          ← исследование + обсуждение
