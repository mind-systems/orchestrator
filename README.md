# Orchestrator

> Автономный оркестратор AI-агентов — планирует, имплементирует и ревьюит задачи из роадмапа.

Orchestrator читает milestone-ы из роадмапа целевого проекта и прогоняет каждый через конвейер из трёх агентов: Planner составляет план, Implementer пишет код, Reviewer проверяет результат. Если ревью находит проблемы — цикл повторяется. В конце каждого milestone оркестратор делает git commit и отмечает задачу выполненной.

## Быстрый старт

```bash
cd orchestrator && uv sync
uv run orchestrator implement /path/to/project
```

Требуется установленный и авторизованный [Claude Code](https://claude.ai/code) CLI.

## Режимы работы

| Команда | Что делает |
|---------|-----------|
| `implement` | Планирует и имплементирует все pending-milestone-ы |
| `review` | Ревьюит все существующие планы против текущей кодовой базы |
| `implement-review` | Имплементирует, затем запускает полный review-проход |
| `refactor` | Аудит и рефакторинг — агент сам находит проблемы и чинит их |

## Документация

| Раздел | Описание |
|--------|----------|
| [Как это работает](docs/how-it-works.md) | Агентный конвейер, сессии, файловый протокол |
| [Режим рефакторинга](docs/refactor-mode.md) | Как агент находит костыли и что происходит при PLAN_FAIL |
| [Конфигурация](docs/configuration.md) | Env-переменные, модели агентов, лимиты итераций |
| [Требования к проекту](docs/target-project.md) | Что нужно целевому проекту для работы с оркестратором |

## Лицензия

MIT
